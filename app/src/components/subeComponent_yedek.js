...
*/
    ...
    onKeyUp = (e) => {
       ...
       /* if (this.refAutoComplete.current.value.length > 0) {
            this.autoCompleteData.forEach((data, i) => {
                let autoCompleteDataVal = data.combinedAutoCompleteVal;
                let inputVal = this.refAutoComplete.current.value.toLocaleLowerCase();

                if (autoCompleteDataVal.toLocaleLowerCase().indexOf(inputVal) > -1) {
                    this.shownValsAutoComplete.push(data);
                    this.forceUpdate();
                }

                // console.log('hello ' , autoCompleteDataVal.toLocaleLowerCase().match(('/'+inputVal+'/g')));

                // let regex = '/'+inputVal+'/g';
                // console.log("regex " , regex);

                // if(autoCompleteDataVal.toLocaleLowerCase().match(('/'+inputVal+'/g'))){
/!*
                if(autoCompleteDataVal.toLocaleLowerCase().match(regex)){
